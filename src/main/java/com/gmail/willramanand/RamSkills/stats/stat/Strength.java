...
public class Strength extends AbstractStat {

    public Strength(RamSkills plugin) {
        super(plugin, Stat.STRENGTH);
    }

    @Override
    public String print(Player player) {
        SkillPlayer skillPlayer = RamSkills.getInstance().getPlayerManager().getPlayerData(player);
        double points = skillPlayer.getStatPoint(Stat.STRENGTH);
        return "+ " + Formatter.decimalFormat(points, 1) + "% Weapon Damage";
    }
}
